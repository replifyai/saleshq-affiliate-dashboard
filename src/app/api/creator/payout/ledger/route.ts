...
export async function GET() {
    try {
        const response = await authedBackendFetch('/payout/ledger', { method: 'GET' });
        if (!response.ok) {
            let message = 'Failed to fetch earnings ledger';
        ...
        return NextResponse.json(
            { error: 'Internal Server Error', message: error instanceof Error ? error.message : 'Failed to fetch earnings ledger', success: false } as ErrorResponse,
            { status: 500 }
        );
    }
}
